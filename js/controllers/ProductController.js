angular.module('FrangiApp').controller('ProductController', ['$scope','$http','$stateParams','$location','$anchorScroll',function($scope,$http,$stateParams,$location,$anchorScroll) {
	...
	function ChangeSize(){
		...
		var headh=$('.header_content').height();
		var bottomh=$('.pfix_bottom').height();
		$(".phome").height(windowh-headh-bottomh);
	}   
	...
	$(".pro_title").click(function() {
		var curnum=$(this).index(".pro_title");
		var curcontent=$(".pro_content:eq(" + curnum + ")");
		if(curcontent.css("display")=='none'){
			$(this).find('.icon_add').css("transform", "rotate(90deg)");
		}else{
			$(this).find('.icon_add').css("transform", "rotate(270deg)");
		}
		curcontent.slideToggle("slow");
	});
	$(".pro_title").click();
	$scope.gotoBuy = function() {
		$location.hash('buy_content');
		$(".pro_title:eq(1)").click();
		$anchorScroll();
	};
}]); 